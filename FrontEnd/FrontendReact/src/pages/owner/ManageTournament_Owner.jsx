...
function OwnerRegistrationsTab({ tournamentId }) {
  ...
  const fetchRegistrations = async () => {
    try {
      setLoadingRegs(true);
      const res = await stadiumOwnerService.getTournamentRegistrations(tournamentId);
  ...
  const handleApprove = async (teamId) => {
    try {
      await stadiumOwnerService.approveRegistration(tournamentId, teamId);
  ...
  const handleReject = async (teamId) => {
    if (!window.confirm('Bạn có chắc muốn từ chối và hoàn tiền?')) return;
    try {
      await stadiumOwnerService.rejectRegistration(tournamentId, teamId);
      alert('Đã từ chối!');
      fetchRegistrations();
    } catch (err) {
      alert('Lỗi: ' + (err.response?.data?.message || err.message));
  ...
  return (
    <div className="animate-fade-in">
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-100 dark:bg-slate-800">
            <tr>
              ...
              <th className="p-3 text-xs font-bold text-slate-600 dark:text-slate-300">Đội trưởng</th>
              <th className="p-3 text-xs font-bold text-slate-600 dark:text-slate-300">Thời gian gửi</th>
              <th className="p-3 text-xs font-bold text-slate-600 dark:text-slate-300 text-right">Hành động</th>
            </tr>
          </thead>
          <tbody>
            {loadingRegs ? (
              <tr><td colSpan="4" className="p-3 text-center text-slate-500">Đang tải...</td></tr>
            ) : registrations.length === 0 ? (
              <tr><td colSpan="4" className="p-3 text-center text-slate-500">Không có đội nào đang chờ duyệt</td></tr>
            ) : (
              registrations.map(r => {
                const hoursDiff = Math.floor((new Date() - new Date(r.registrationDate)) / (1000 * 60 * 60));
                return (
                  <tr key={r.teamId} className="border-t border-slate-200 dark:border-slate-700">
                    <td className="p-3 font-bold text-slate-800 dark:text-white">{r.teamName}</td>
                    <td className="p-3 text-sm text-slate-600 dark:text-slate-400">{r.captainName}</td>
                    <td className="p-3 text-sm">
                      <span className={`px-2 py-1 rounded font-bold ${hoursDiff > 20 ? 'text-rose-600 bg-rose-50' : 'text-amber-600'}`}>
                        {hoursDiff}h trước {hoursDiff > 20 && '(Sắp vi phạm SLA)'}
                      </span>
                    </td>
                    <td className="p-3 text-right flex justify-end gap-2">
                      <button onClick={() => handleReject(r.teamId)} className="px-3 py-1.5 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg">Từ Chối</button>
                      <button onClick={() => handleApprove(r.teamId)} className="px-3 py-1.5 text-xs font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg shadow-sm">Duyệt & Thêm</button>
                    </td>
                  </tr>
                )
              })
      ...
      <p className="text-xs text-slate-500 mt-2 flex items-center gap-1">
        ...
        Lưu ý: Yêu cầu đăng ký quá 24h không duyệt sẽ tự động hoàn tiền và tài khoản Chủ sân sẽ bị ghi nhận vi phạm.
      </p>
    </div>
  );
}
...
export default function ManageTournament_Captain() {
  ...
  const loadTournamentSettings = async () => {
    try {
      const [data, stadiums] = await Promise.all([
        stadiumOwnerService.getTournamentSettings(id).catch(() => null),
        stadiumOwnerService.getMyStadiums().catch(() => ({ data: [] }))
      ]);
      if (data) {
        setSettings(data);
      }
      setMyStadiums(stadiums.data || []);
    } catch (e) {
  ...
  const loadTeams = async () => {
    try {
      const data = await stadiumOwnerService.getTournamentTeams(id);
  ...
  const loadBracket = async () => {
    try {
      const data = await stadiumOwnerService.getTournamentBracket(id);
  ...
  const handleGenerateInternalTeams = async () => {
    if (confirm(`Bạn muốn tự động chia thành viên hiện tại thành ${numInternalTeams} đội?`)) {
      setLoading(true);
      await stadiumOwnerService.generateInternalTeams(id, numInternalTeams);
  ...
  const handleSubmitAddTeam = async (e) => {
    ...
    if (addTeamForm.isNew) {
      await stadiumOwnerService.addTeamToTournament(id, { name: addTeamForm.name, abbr: addTeamForm.abbr.toUpperCase() });
    } else {
      const selected = systemTeams.find(t => t.id === Number(addTeamForm.selectedTeamId));
      if(selected) {
        await stadiumOwnerService.addTeamToTournament(id, { teamId: selected.id, name: selected.name, abbr: addTeamForm.abbr.toUpperCase() });
      }
    }
    await loadTeams();
  ...
  const handleSaveSwissRound = async (roundNum, matches) => {
    setLoading(true);
    await stadiumOwnerService.updateSwissMatches(id, roundNum, matches);
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="p-8">
            ...
            {activeTab === 'matches_list' && (
              <TournamentMatchesList tournamentId={id} service={stadiumOwnerService} />
            )}
