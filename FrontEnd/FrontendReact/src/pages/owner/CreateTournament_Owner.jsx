...
export default function CreateTournament_Owner() {
  ...
  useEffect(() => {
    const fetchStadiums = async () => {
      try {
        ...
        setMyStadiums(res.data || []);
        if (res.data && res.data.length > 0) {
          setFormData(prev => ({ ...prev, stadium: res.data[0].stadiumName }));
        }
      } catch (err) {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!formData.stadium) {
      Swal.fire('Lỗi', 'Vui lòng chọn sân thi đấu.', 'error');
      return;
    }
    ...
    try {
      ...
      await stadiumOwnerService.createTournament(payload);
      Swal.fire('Thành công', 'Tạo giải đấu thành công! Vui lòng chờ admin duyệt.', 'success');
      navigate('/owner-home?tab=tournaments');
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-wc-navy-900 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate('/owner-home?tab=tournaments')}
            className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
          >
            <FiArrowLeft size={24} className="text-slate-600 dark:text-slate-300" />
          </button>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <FaTrophy className="text-emerald-500" /> Khởi tạo Giải Đấu (Chủ Sân)
          </h1>
        </div>
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <div 
                      key={pkg.teams}
                      className={`p-6 rounded-2xl border-2 cursor-pointer transition-all ${formData.maxTeams === pkg.teams ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 shadow-md' : 'border-slate-200 dark:border-slate-700 hover:border-emerald-300'}`}
                ...
                <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800 mt-6">
                  <button type="button" onClick={() => navigate('/owner-home?tab=tournaments')} className="px-8 py-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition-colors">Hủy bỏ</button>
                  <button type="button" onClick={() => setStep(2)} className="px-8 py-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold rounded-xl shadow-lg hover:-translate-y-0.5 transition-all">Tiếp tục</button>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div>
                  ...
                  <input type="text" required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-shadow" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      required
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white outline-none transition-shadow focus:ring-2 focus:ring-emerald-500"
                      value={formData.stadium}
                      onChange={e => setFormData({...formData, stadium: e.target.value})}
                    >
                      <option value="">-- Chọn sân của bạn --</option>
                      {myStadiums.map(s => (
                        <option key={s.stadiumId || s.id} value={s.stadiumName}>{s.stadiumName}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                {formData.scope !== 'Internal' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input type="number" required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-shadow" value={formData.entryFee} onChange={e => setFormData({...formData, entryFee: Number(e.target.value)})} placeholder="Ví dụ: 500000" />
                    </div>
                    <div>
                ...
                <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800 mt-8">
                  ...
                  <button type="button" onClick={() => { if(!formData.name || !formData.startDate || !formData.endDate || !formData.stadium) Swal.fire('Lỗi', 'Vui lòng điền đủ thông tin', 'warning'); else setStep(3); }} className="px-8 py-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold rounded-xl shadow-lg hover:-translate-y-0.5 transition-all">Tiếp tục</button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6 animate-fade-in">
                ...
                <div>
                  ...
                  <input type="text" required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white outline-none transition-shadow focus:ring-2 focus:ring-emerald-500" value={formData.organizerCccd} onChange={e => setFormData({...formData, organizerCccd: e.target.value})} placeholder="Nhập 12 số CCCD" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Số Bằng lái xe (Tùy chọn)</label>
                  <input type="text" className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white outline-none transition-shadow focus:ring-2 focus:ring-emerald-500" value={formData.organizerDriverLicense} onChange={e => setFormData({...formData, organizerDriverLicense: e.target.value})} placeholder="Nhập số giấy phép lái xe" />
                </div>
                <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 mt-6 shadow-sm">
                  <label className="flex items-start gap-4 cursor-pointer">
                    <input type="checkbox" className="mt-1 w-6 h-6 rounded border-slate-300 text-emerald-500 focus:ring-emerald-500 bg-white dark:bg-slate-900" checked={formData.agreeTerms} onChange={e => setFormData({...formData, agreeTerms: e.target.checked})} />
